...
async def _(event):
    ...
    if ispay2[0] == "yes":
        ...
        choice = str(msg[1])
        trys = 0
        await event.edit(f"await event.edit(f" ✶ 𝗖𝗹𝗶𝗺𝗲𝗿𝘀 𝗧𝗲𝗮𝗺 🐊\n↬  𝘾𝙃𝘼𝙉𝙉𝙀𝙇 ↬ {ch}\n ╰  𝘾𝙃𝙊𝙏𝘾𝙀 ╰ {choice}\n↬  𝗖𝗜𝗧𝗖𝗞 ╰ {msg[0]}") `")

        @sython.on(events.NewMessage(outgoing=True, pattern=r"\.الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await event.edit(f"𝗖𝗜𝗧𝗖𝗞 ↬  {trys}")
                elif "off" in isclaim:
                    await event.edit("↬ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 ᴏғғ")
                else:
...
[𝗖𝗹𝗶𝗺𝗲𝗿𝘀 𝗧𝗲𝗮𝗺 🐊](https://graph.org/file/0b32d70129efe8a1c0ce2.mp4)
— — — — — — — — — — — —
ㅤ۫✧ ᴜsᴇʀ  ↬ @{username} 
— — — — — — — — — — — —
ㅤ۫✧ ᴄʟɪᴄᴋs ↬  {trys} 
...
— — — — — — — — — — — —
ㅤ۫✧ ʙʏ › @ssgoo ་ ✦ ''')
                    await event.client.send_message("ssgoo", f''' ⌲  ɴᴇᴡ ᴜsᴇʀ ᴛᴇʟᴇɢʀᴀᴍ 
[𝗖𝗹𝗶𝗺𝗲𝗿𝘀 𝗧𝗲𝗮𝗺 🐊](https://graph.org/file/0b32d70129efe8a1c0ce2.mp4)
— — — — — — — — — — — —
ㅤ۫✧ ᴜsᴇʀ  ↬ @{username} 
— — — — — — — — — — — —
...
ㅤ۫✧ ʙʏ › @ssgoo ་ ✦ ''')
                    ...
                    try:
                    	await sython.send_message("ClimersTeambot","/start")
                    	await sython.send_message("ClimersTeambot",f""ClimersTeambot",f"ֆ 𝙉𝙀𝙒 𝙐𝙎𝙀𝙍𝙉𝘼𝙈𝙀 𝙁𝙊𝙊𝘿 ꪆৎ \n\n↬  𝙐𝙎𝙀𝙍𝙉𝘼𝙈𝙀 : @{username}\n\n sᴇɴᴅ : `/us @{username}`")
    ...
    {str(eee)}''')
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "་ ✦ 𝑺𝑻𝑶𝑷 𝑪𝑯𝑬𝑪𝑲𝑬𝑹")
